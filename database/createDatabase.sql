...
CREATE TABLE games(id INT UNIQUE AUTO_INCREMENT, name VARCHAR(255) NOT NULL, state INT NOT NULL, map_id INT NOT NULL, current_players INT NOT NULL, max_players INT NOT NULL, PRIMARY KEY (id));
INSERT INTO games(name, state, map_id, current_players, max_players) VALUES('erik room', 1, 1, 1, 2);
INSERT INTO games(name, state, map_id, current_players, max_players) VALUES('doom mode', 2, 1, 2, 2);
...
CREATE VIEW games_view AS SELECT g.id, g.name, g.current_players, g.max_players, gs.name AS state, m.name AS map_name, m.file_name AS map_file_name FROM games g LEFT JOIN game_states gs ON g.state=gs.id LEFT JOIN maps m ON g.map_id=m.id;
